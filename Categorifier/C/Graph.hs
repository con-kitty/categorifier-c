...
module Categorifier.C.Graph (Graph (..), GraphFailure (..)) where

import Categorifier.C.CExpr.Types (CExprTypeProduct (..))
...
--
-- Type parameter @e@ is usually instantiated to 'Categorifier.C.CExpr.Types.Core.CExprF'.
--
-- == Invariants
--
...
data Graph (e :: (Type -> Type) -> Type -> Type) = Graph
  { -- | The nodes: for each primitive type, 'Categorifier.C.CExpr.Types.Core.CExprFunctionCall' and
    -- 'Categorifier.C.CExpr.Types.Core.CExprSelectExpression', there is a map from the unique node ID
    -- to that node.
