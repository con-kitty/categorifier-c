...
module Categorifier.C.KTypes.SwitchCase
  ( KIf (..),
    switch,
    switch',
    unsafeIndex,
...
import Categorifier.C.KTypes.KEnum (KEnum (..))
import Categorifier.C.KTypes.KLiteral (KLiteral (..), true)
import Categorifier.C.PolyVec (PolyVec, pdevectorize, pvectorize)
