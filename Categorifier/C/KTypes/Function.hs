...
module Categorifier.C.KTypes.Function
...
-- | Given a proxy to the @f@ container type (e.g. 'Categorifier.C.CExpr.Types.Core.CExpr' or
-- 'Categorifier.C.KTypes.C.C') @fProxy@, a function name @functionName@ and a Haskell function
-- @myfunction@,
--
...
class KVariadicVectorizeFunctionInputs (isFunCall :: FunCall) f r where
  vectorizeFunctionInputs ::
    ...
    -- to the function, so that at the base case, it will 'Categorifier.C.PolyVec.pvectorize' all the inputs
