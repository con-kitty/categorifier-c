...
-- | Primitive types for all code generation systems.
--
...
-- Type@ like a normal list (@[]@) or 'Data.Vector.Vector'.  The @Categorifier.C.HK1@ library exists to make
...
-- these types, you may want to read through the docs for @Categorifier.C.HK1@ first, both to familiarize
...
module Categorifier.C.Prim.Base
  ( -- * Primitive types
    Primitives,
    Prim (..),
    Arrays (..),
...
-- @`Prim` `Categorifier.C.KTypes.C.C`@ and so on.
deriving instance (forall a. Show a => Show (f a)) => Show (Prim f)

-- | A product of all primitive types in the 'Categorifier.C.PrimTypes.K' DSL.
--
...
-- | Manual instance for Categorifier.C performance. This should be eliminated when we replace PolyVec
...
makeLensesWith categorifierLensRules ''Arrays
...
class
  ...
  -- @-XTypeApplications@; for example, 'Categorifier.C.Prim.Types.primTypeProxy' is defined as
  --
  ...
  -- use 'Categorifier.C.Prim.Types.getPrimHaskellName'.)
...
-- ==== 'Arrays' helpers
--
-- Most of the helper types like 'Categorifier.C.Prim.ArrayCount.ArrayCount' or
-- 'Categorifier.C.Prim.Compose.Compose' provide 'Control.Lens.Type.Iso's
-- ('Categorifier.C.Prim.ArrayCount._ArrayCount'; 'Categorifier.C.Prim.Compose._Compose') and
...
--
-- Similarly, 'Control.Lens.At.Ixed' support is provided for 'Categorifier.C.Prim.Compose.Compose', so you
...
-- | This is a customized version of `Categorifier.C.TypeUtils.Lists.IsElem` which lets us give slightly more
