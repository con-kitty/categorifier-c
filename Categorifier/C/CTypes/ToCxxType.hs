...
module Categorifier.C.CTypes.ToCxxType
  ( ToCxxType (..),
    ToCxxTypeError (..),
...
import Categorifier.C.CTypes.KTypeRep (KTypeRep, toKTypeRep)
...
import Categorifier.Duoidal (traverseD)
...
-- use the `Categorifier.C.CTypes.CGeneric.CGenerically` wrapper newtype to guide derivation of this class as
-- part of the transition from `CxxType` to `Categorifier.C.CTypes.CGeneric.CGeneric`.  Unfortunately, that
-- wrapper interacts poorly with our parameter @g@ here, and we can't do anything about it at the
-- use site (in "Categorifier.C.CTypes.CGeneric.CType") because @g@ can't be referred to from the class
...
class SupportsKBits f => ToCxxType f a where
  ...
  default toCxxType ::
    ( Applicative g,
      ...
      -- this once 'Categorifier.PolyVec.PolyVec' is completely replaced by 'CType'.
    ) =>
    g a ->
    CxxType (Compose g f)
