...
module Categorifier.C.CTypes.CGeneric
...
import Categorifier.C.CTypes.CGeneric.Class
  ( Array (..),
    ArraySize,
    Bitfield (..),
    Bits (..),
...
import qualified Categorifier.C.Client.Internal as ConCat
import qualified Categorifier.C.KTypes.KBits as KBits
import Categorifier.C.Nat.Operators (type (*), type (+), type (<=))
...
-- modules (e.g. "Categorifier.C.CTypes.CGeneric.Serialise").  It is the `CGeneric` version of the
...
--
-- == Usage examples from the "Categorifier.C.CTypes.CGeneric.Test" module
--
-- This section shows how to use `CGenerically` with @-XDerivingVia@ to derive instances.
--
-- > import qualified Codec.Serialise as Serialise
-- > import qualified Categorifier.C.CTypes.CGeneric.Serialise as CG
-- > import Categorifier.C.CTypes.CGeneric (CGeneric (..), CGenerically (..))
--
-- === Simple types
--
...
-- === Higher-kinded data
--
-- A type parameterized over `Categorifier.C.KTypes.C.C` and similar type constructors typically
-- needs to have its parameter specialized to `Categorifier.C.KTypes.C.C`, which can be done in a
...
-- the "Categorifier.C.CTypes.CGeneric.Serialise" module, `Categorifier.C.CTypes.CGeneric.Serialise.serialise` is
-- defined using `CGenerically`:
--
-- > serialise :: Serialise a => a -> ByteString
-- > serialise = Serialise.serialise . CGenerically
--
-- The important thing here is that the unqualified `Categorifier.C.CTypes.CGeneric.Serialise.Serialise`
...
-- `Rep`, stripping off any wrappers it finds (e.g. `Categorifier.C.KTypes.C.C` or
...
-- documentation.
--
-- > import Categorifier.C.CTypes.CGeneric (CGeneric, AsBitfield (..))
-- >
...
-- complete instance for `Categorifier.Config.Types.Rotors` consists of the following:
--
...
--
-- > import Categorifier.C.CTypes.CGeneric (CGeneric, AsArray (..))
-- >
-- > data Rotors a = Rotors
-- >   { rotorA :: a,
-- >     rotorB :: a,
