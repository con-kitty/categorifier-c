...
module Categorifier.C.Graph.Reify
  ( -- * Graph type
    Graph (..),
...
import Categorifier.C.Graph (Graph (..))
import Categorifier.C.Higher (HTraversableC (..))
import Categorifier.C.Prim (Arrays)
import Categorifier.C.Recursion (NaturalTransformation, Projectable, hproject)
