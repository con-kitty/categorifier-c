module Kitty.KGenGenerate.FFI.Bindings
  ( mkFFIBindingModule,
    ffiBindingModule,
  )
where
...
import Kitty.Codegen.FFI.Spec (SBVSpec)
...
ffiBindingModule funName spec = Module () hd prags imports decls
  where
    ...
    imports =
      [ ImportDecl
          ()
          (ModuleName () "Kitty.Codegen.FFI.Spec")
          False
          False
          False
          Nothing
