...
import Kitty.CExpr.C
  ( genAssignment,
    genFunctionDeclaration,
    genFunctionDefinition,
    genOutputAssignment,
  )
import Kitty.CExpr.C.Assignment (ArrayIndexOffset, AssignState (..), AssignVar)
import Kitty.CExpr.Types (CExprTypeLens, CExprTypeProduct (..))
import Kitty.CExpr.Types.Core (CExprF (..), Callee (..))
import Kitty.Graph (Graph, GraphFailure (..))
import Kitty.Graph.Fold (nodeHezygoM)
import Kitty.Higher (HFunctorC (hmapC))
import Kitty.Prim
  ( Arrays,
    IsPrimitive,
  )
