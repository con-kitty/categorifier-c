...
import Kitty.CExpr.C.Assignment
  ( AssignState (..),
    AssignTypeError (..),
    AssignVar,
    prettyAssignVar,
  )
import Kitty.CExpr.C.Operations
  ( genBoolBinOp,
    genBoolUnOp,
    genCast,
    genCastFP,
...
import Kitty.CExpr.Types (CmpOp (..))
  ...
  where
    ...
    mkArg label (Const n) =
      let typeName = genTypeName @a
          ...
          -- when we generate it!
          --
          -- This is a temporary solution; see https://kitty-hawk.atlassian.net/browse/SW-943 for
          -- more information.
          unused = "__attribute__((unused))"
       in pure $ typeName <+> label <> "_" <> typeName <> Doc.brackets (Doc.pretty n) <+> unused
    -- See https://kitty-hawk.atlassian.net/browse/SW-942
