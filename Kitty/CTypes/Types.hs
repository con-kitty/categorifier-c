...
import Kitty.CTypes.KTypeRep (KTypeRep)
import Kitty.KTypes.KBits (KBits, setBitTo, testBit)
import Kitty.KTypes.KLiteral (KLiteral, kliteral)
import Kitty.Prim (IsPrimitive, Prim (..))
import Kitty.Recursion (BFix, hembed)
import PyF (fmt)
...
-- | 'SupportsKBits' exists because when we call 'Kitty.CTypes.ToCxxType.ToCxxType' on bitvectors,
-- it converts the bits to a list of 'Bool's. I'm not sure if this is the best way or not but it
-- works well enough for now. It makes all 'Kitty.CTypes.ToCxxType.ToCxxType' calls require the
-- constraint though.  TODO(greg): Once 'Kitty.PolyVecPolyVec' is gone and the design is complete,
...
-- comparison code, we need this instance, which exists only for `Kitty.KTypes.C.C` and
...
-- 'Kitty.Math.Linear.Vectorize.vlength'.
data CNat
