...
module Kitty.CTypes.CGeneric.Class
...
-- | This type wraps a product of fields of type `Bool`.  See `Kitty.CTypes.CGeneric.RepBitfield`
...
-- > = D1
-- >   ('MetaData "Switches" "Kitty.CTypes.CGeneric" "main" 'False)
-- >   (C1
...
--
-- If you are used to the `Kitty.CTypes.Types.CxxType` way of representing an enum as an
-- `Kitty.CTypes.Types.CEnum`, just keep in mind that a `Word8` with no wrapper type @f@ is
...
-- @`Kitty.CTypes.Types.CxxType` `Proxy`@; `CGeneric` represents this structural information at the
-- type level instead.
...
-- the original `G.Rep` of the structure's fields.  See `Kitty.CTypes.CGeneric.RepArray` for
...
-- array using `Kitty.CTypes.CGeneric.RepArray` or `Kitty.CTypes.CGeneric.AsArray`.
--
...
-- `Kitty.CTypes.CGeneric.RepFlattenedArray` or `Kitty.CTypes.CGeneric.AsFlattenedArray`, where
...
--
-- Primitive C types may also appear inside their own wrappers such as `Kitty.KTypes.C.C`.
-- In this case, one sees e.g. @`Prim (`Kitty.KTypes.C.C` `Bool`) p@.  To write
...
-- >   type Rep (C Bool) =
-- >     PrimRep "CBool" "Kitty.KTypes.C" "ktypes" (C Bool)
-- >   from = primFrom
-- >   to = primTo
...
-- | This newtype wrapper is analogous to `Kitty.CTypes.CGeneric.AsBitfield` or
-- `Kitty.CTypes.CGeneric.AsArray`: it can be used with @deriving via@ to provide field names in C
...
instance CGeneric (Vec n elem) where
  type
    Rep (Vec n elem) =
      D1
        ('MetaData "Vec" "Kitty.Indexed.Vec" "commonZSindexedZSvec" 'False)
