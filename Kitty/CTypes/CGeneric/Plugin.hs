...
--
--   However, we can't use that in the case where we're dealing with `Kitty.CExpr.Types.Core.CExpr`
--   instead of `Kitty.KTypes.C.C`, because the underlying @a@ hardcodes
--  `Kitty.KTypes.C.C`, so we use this as a bypass.
module Kitty.CTypes.CGeneric.Plugin
  ( RepRep (..),
    repRep,
    repRep',
    Vectorizable (..),
...
import Kitty.Nat.Operators (type (+))
...
-- | This handles the `Kitty.CExpr.Types.Core.CExpr` / `Kitty.KTypes.C.C` mismatch case by
