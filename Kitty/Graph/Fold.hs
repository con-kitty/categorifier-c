...
import Kitty.Duoidal.Either (noteAccum)
import Kitty.Graph (Graph (..), GraphFailure (..))
import Kitty.Higher (HFunctorC (hmapC), HTraversableC (htraverseC))
...
--
-- This is the higher-kinded counterpart of @Kitty.Graph.Fold.cataM'@.
nodeHcataM' ::
  forall
    (m :: Type -> Type)
