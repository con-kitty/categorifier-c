...
import Kitty.CExpr.Cat (Cat (..), ToTargetOb (..), castK, cat)
import Kitty.CExpr.Cat.TargetOb (TargetOb, TargetObTC1, TargetObW (..))
import Kitty.CExpr.Types (CExprTypeLens)
...
import Kitty.CTypes.GArrays (FromArraysError, GArrays (..))
import Kitty.CTypes.Instances (PrimitivesToCxxType)
import Kitty.CTypes.ToCxxType (CoercibleT, ToCxxType (..))
import Kitty.CTypes.Types (CxxType, SupportsKBits)
import Kitty.Cat.Category (IntegralCat' (..), RealToFracCat (..))
...
import Kitty.KTypes.Function (KForeignFunctionCall (..), KFunCall (..))
...
-- @Kitty.CTypes.CGeneric.CType.constructorToConPrim@ and
-- @Kitty.CTypes.CGeneric.CType.datatypeToCTypePrim@
...
instance
  ...
  NativeCat Cat "Kitty.KTypes.Round.kRoundDouble" (C Double) (C a)
  where
...
instance CGeneric (C Bool) where
  type Rep (C Bool) = CG.PrimRep "CBool" "Kitty.KTypes.C" "ktypes-native" (C Bool)
  from = CG.primFrom
  to = CG.primTo

instance CGeneric (C Int8) where
  type Rep (C Int8) = CG.PrimRep "CInt8" "Kitty.KTypes.C" "ktypes-native" (C Int8)
  from = CG.primFrom
  to = CG.primTo

instance CGeneric (C Int16) where
  type Rep (C Int16) = CG.PrimRep "CInt16" "Kitty.KTypes.C" "ktypes-native" (C Int16)
  from = CG.primFrom
  to = CG.primTo

instance CGeneric (C Int32) where
  type Rep (C Int32) = CG.PrimRep "CInt32" "Kitty.KTypes.C" "ktypes-native" (C Int32)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (C Int64) where
  type
    Rep (C Int64) =
      CG.PrimRep "CInt64" "Kitty.KTypes.C" "ktypes-native" (C Int64)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (C Word8) where
  type
    Rep (C Word8) =
      CG.PrimRep "CWord8" "Kitty.KTypes.C" "ktypes-native" (C Word8)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (C Word16) where
  type
    Rep (C Word16) =
      CG.PrimRep "CWord16" "Kitty.KTypes.C" "ktypes-native" (C Word16)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (C Word32) where
  type
    Rep (C Word32) =
      CG.PrimRep "CWord32" "Kitty.KTypes.C" "ktypes-native" (C Word32)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (C Word64) where
  type
    Rep (C Word64) =
      CG.PrimRep "CWord64" "Kitty.KTypes.C" "ktypes-native" (C Word64)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (C Float) where
  type
    Rep (C Float) =
      CG.PrimRep "CFloat" "Kitty.KTypes.C" "ktypes-native" (C Float)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (C Double) where
  type
    Rep (C Double) =
      CG.PrimRep "CDouble" "Kitty.KTypes.C" "ktypes-native" (C Double)
  from = CG.primFrom
  to = CG.primTo
