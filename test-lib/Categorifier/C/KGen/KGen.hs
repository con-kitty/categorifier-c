...
import qualified Categorifier.C.KGen.TH as KGen
import Categorifier.C.KTypes.ArcTan2 (ArcTan2 (..))
import qualified Categorifier.C.KTypes.BooleanLogic as KT (KAnd (..))
import qualified Categorifier.C.KTypes.Conditional as KT (KSelect (..), KTernary (..))
import Categorifier.C.KTypes.Equality (KEq (..))
import Categorifier.C.KTypes.FMod (FMod (..))
import qualified Categorifier.C.KTypes.FromIntegral as KT (KFromIntegral (..))
import qualified Categorifier.C.KTypes.Function as KT
import qualified Categorifier.C.KTypes.IEEE as KT (KConvertFloat (..), KIsInfinite (..), KIsNaN (..))
import qualified Categorifier.C.KTypes.KBits as KT
import Categorifier.C.KTypes.KDivisible (KDivisible (..))
import Categorifier.C.KTypes.KLiteral (KLiteral (..))
import qualified Categorifier.C.KTypes.KType1 as KT (KType1)
import qualified Categorifier.C.KTypes.Libm as Libm
import Categorifier.C.KTypes.Round (KRound (..))
import qualified Categorifier.C.KTypes.Round as Round
...
instance CGeneric (KGen Bool) where
  type
    Rep (KGen Bool) =
      CG.PrimRep "KGenBool" "Categorifier.C.KTypes.KGen" "kgen" (KGen Bool)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (KGen Int8) where
  type
    Rep (KGen Int8) =
      CG.PrimRep "KGenInt8" "Categorifier.C.KTypes.KGen" "kgen" (KGen Int8)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (KGen Int16) where
  type
    Rep (KGen Int16) =
      CG.PrimRep "KGenInt16" "Categorifier.C.KTypes.KGen" "kgen" (KGen Int16)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (KGen Int32) where
  type
    Rep (KGen Int32) =
      CG.PrimRep "KGenInt32" "Categorifier.C.KTypes.KGen" "kgen" (KGen Int32)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (KGen Int64) where
  type
    Rep (KGen Int64) =
      CG.PrimRep "KGenInt64" "Categorifier.C.KTypes.KGen" "kgen" (KGen Int64)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (KGen Word8) where
  type
    Rep (KGen Word8) =
      CG.PrimRep "KGenWord8" "Categorifier.C.KTypes.KGen" "kgen" (KGen Word8)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (KGen Word16) where
  type
    Rep (KGen Word16) =
      CG.PrimRep "KGenWord16" "Categorifier.C.KTypes.KGen" "kgen" (KGen Word16)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (KGen Word32) where
  type
    Rep (KGen Word32) =
      CG.PrimRep "KGenWord32" "Categorifier.C.KTypes.KGen" "kgen" (KGen Word32)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (KGen Word64) where
  type
    Rep (KGen Word64) =
      CG.PrimRep "KGenWord64" "Categorifier.C.KTypes.KGen" "kgen" (KGen Word64)
  from = CG.primFrom
  to = CG.primTo
...
instance CGeneric (KGen Float) where
  type
    Rep (KGen Float) =
      CG.PrimRep "KGenFloat" "Categorifier.C.KTypes.KGen" "kgen" (KGen Float)
  from = CG.primFrom
  to = CG.primTo
