...
-- function. See 'module Categorifier.C.Codegen.FFI.Call' for functions that can do
-- this.
module Categorifier.C.Codegen.FFI.Spec
  ( -- * FFI call types
    SBVGetSpecSize,
    SBVGetSpec,
    SBVFunCall,
...
import Categorifier.C.Prim (ArrayCount (..), Arrays (..), countLenses)
import qualified Categorifier.Common.IO.Exception as Exception (Exception)
...
-- | The type of a function call to 'Categorifier.C.CExpr.Types.Core.CExpr'-produced C code
-- at the FFI boundary.
type SBVFunCall =
  -- Boolean inputs
  Ptr CBool ->
...
makeLensesWith categorifierLensRules ''Spec
...
makeLensesWith categorifierLensRules ''SBVCFunction
...
makeLensesWith categorifierLensRules ''SpecMismatchInfo
...
-- 'Categorifier.C.PolyVec.PolyVec.pvlengths' into the "spec" for an FFI call.
--
-- This is needed because we can never use empty arrays with SBV-generated C code, but all
-- 'Categorifier.C.CExpr.Types.Core.CExpr'-related C functions have an identical calling convention, wherein
