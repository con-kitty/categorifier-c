...
-- | FFI calls to dynamically-loaded 'Categorifier.C.KGen.KGen' functions using @dlopen@ and friends.
module Categorifier.C.KGenGenerate.FFI.Plugin
  ( -- * Temporary files
    getTempDirectory,
...
import Categorifier.C.Codegen.FFI.Foreign (ptrToFunSBV, ptrToFunSBVSpec, ptrToFunSBVSpecSize)
...
import qualified Categorifier.Common.IO.Exception as Exception (Exception, bracket)
...
unsafeLoadPlugin ::
  ...
  -- 'Categorifier.C.FFI.KGen.codeGenWithBindings')
  SBVSpec ->
